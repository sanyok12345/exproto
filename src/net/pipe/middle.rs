            ...
            result = client.read(&mut buf) => {
                match result {
                    Ok(n @ 1..) => {
                        ...
                        if middle.send_proxy_req(conn_id, peer, our_addr, proto_tag, ad_tag, &buf[..n]).await.is_err() { break; }
...
pub async fn relay_faketls(
    ...
    if let Some(mut extra) = initial_extra {
        ...
        if middle.send_proxy_req(conn_id, peer, our_addr, proto_tag, ad_tag, &extra).await.is_err() { return; }
    }
            ...
            result = read_record(client) => {
                match result {
                    Ok(data) if !data.is_empty() => {
                        ...
                        if middle.send_proxy_req(conn_id, peer, our_addr, proto_tag, ad_tag, &buf).await.is_err() { break; }
